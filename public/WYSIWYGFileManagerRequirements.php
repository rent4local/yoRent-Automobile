<?php
    ...
    $rootLocation = WEBSITEROOT_LOCALPATH;
    $defaultUploadPath = $uploadDir . 'editor';
    ...
    $sellerSession = 'yorentUserSession';
    $adminSession = 'yorentAdmin';
    ...
    } elseif ($is_admin_for_file_manager) {
    ...
    if (!file_exists($rootLocation . $path_for_images)) {
        //create the folder
        //$dir_to_create = ealpath(dirname(__FILE__). '/../').$path_for_images;
        //@mkdir($dir_to_create, 0777, true);
        //create the folder
        $dir_to_create = $rootLocation.$path_for_images;
