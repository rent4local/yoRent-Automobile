...
class ShopsController extends MyAppController
{
    ...
    public function index()
    {
        ...
        $this->set('title', Labels::getLabel('Lbl_All_Shops', $this->siteLangId));
    ...
    public function featured()
    {
        ...
        $this->set('title', Labels::getLabel('Lbl_Featured_Shops', $this->siteLangId));
        $this->set('searchForm', $searchForm);
    ...
    public function search()
    {
        ...
        $srch->addMultipleFields(
            array(
                ...
                'IFNULL(ufs.ufs_id, 0) as is_favorite', 's.shop_lat', 's.shop_lng'
            )
        );
        ...
        $productSrchObj->addMultipleFields(
            array(
                ...
                'IFNULL(sprodata_is_for_sell, 0) as is_sell', 'IFNULL(sprodata_is_for_rent, 0) as is_rent', 'sprodata_rental_price as rent_price', 'sprodata_rental_stock'
            )
        );
    ...
    public function policy($shop_id)
    {
        ...
        $searchFrm = Common::getSiteSearchForm();
        $searchFrm->fill($frmData);

        $shop = Shop::getAttributesById($shop_id,['shop_user_id']);
        $socialPlatforms = $this->getShopSocialPlatforms($shop['shop_user_id']);
        
        $this->set('socialPlatforms', $socialPlatforms);
        $this->set('frmProductSearch', $frm);
        $this->set('postedData', $frmData);
    ...
    public function sendMessage($shop_id, $selprod_id = 0)
    {
        ...
        $shop = $this->getShopInfo($shop_id,true);
        if (!$shop) {
        ...
        $loggedUserData = $userObj->getUserInfo(array('user_id', 'user_name', 'credential_username', 'credential_verified'), true, false);

        if ($loggedUserData['credential_verified'] == applicationConstants::INACTIVE) {
            Message::addErrorMessage(Labels::getLabel('LBL_User_not_verified,_Not_Allowed_to_message', $this->siteLangId));
    ...
    public function banner($shopId, $sizeType = '', $prodCatId = 0, $lang_id = 0)
    {
        ...
        $defaultImage = 'shop_banner.png';
        switch (strtoupper($sizeType)) {
            case 'THUMB':
                $w = 250;
                $h = 100;
                AttachedFile::displayImage($image_name, $w, $h, $defaultImage);
                break;
            case 'WIDE':
                $w = 1320;
                $h = 320;
                ...
                break;
            default:
                AttachedFile::displayOriginalImage($image_name, $defaultImage);
                break;
        }
    }

    private function getShopInfo($shop_id, $includeFav = false)
    {
        $db = FatApp::getDb();
        $shop_id = FatUtility::int($shop_id);
        $srch = new ShopSearch($this->siteLangId);
        ...
        if ($includeFav) {
            ...
            $favSrchObj->addCondition('ufs_shop_id', '=', $shop_id);
            $srch->joinTable('(' . $favSrchObj->getQuery() . ')', 'LEFT OUTER JOIN', 'ufs_shop_id = shop_id', 'ufs');
            array_push($attr,"IFNULL(ufs.ufs_id, 0) as is_favorite");
        }
    ...
    public function getShopSocialPlatforms($userId)
    {
        $shopUserId = FatUtility::int($userId);
        if ($userId < 1) {
            return [];
        }
        $srchSplat = SocialPlatform::getSearchObject($this->siteLangId);
