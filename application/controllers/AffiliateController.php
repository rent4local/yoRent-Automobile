...
;
    ...
    public function addressInfo()
    {
        ...
        $srch->addCondition('user_id', '=', 'mysql_func_'. $loggedUserId, 'AND', true);
        $rs = $srch->getResultSet();
    ...
    public function userSearch()
    {
        ...
        if ($user_id > 0) {
            $srch->addCondition('user_id', '=', 'mysql_func_'. $user_id, 'AND', true);
        } else {
        ...
        if ($user_active > -1) {
            $srch->addCondition('uc.credential_active', '=', 'mysql_func_'. $user_active, 'AND', true);
        }

        $user_verified = FatApp::getPostedData('user_verified', FatUtility::VAR_INT, -1);
        if ($user_verified > -1) {
            $srch->addCondition('uc.credential_verified', '=', 'mysql_func_'. $user_verified, 'AND', true);
        }
