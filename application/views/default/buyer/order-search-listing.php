    ...
    foreach ($orders as $sn => $order) {
        ...
        foreach ($arr_flds as $key => $val) {
            ...
            switch ($key) {
                ...
                case 'total':
                    $txt = '';
                    $txt .= CommonHelper::displayMoneyFormat(CommonHelper::orderProductAmount($order) + $order['addon_amount']);
                    $td->appendElement('plaintext', array(), $txt, true);
                    break;
                case 'status':
                ...
                case 'action':
                    ...
                    if ($order['opd_sold_or_rented'] == applicationConstants::ORDER_TYPE_RENT && (strtotime($order['opd_rental_start_date']) <= strtotime(date('Y-m-d')))) {
                        $canReturnRefund = false;
                    }

                    if ($canReturnRefund && ($order['return_request'] == 0 && $order['cancel_request'] == 0) && ($order['opd_sold_or_rented'] == applicationConstants::ORDER_TYPE_RENT || ($order['opd_sold_or_rented'] == applicationConstants::ORDER_TYPE_SALE && $returnAge >= $deliveredDaysSpent)) && $order['opd_extend_from_op_id'] < 1 && $order['opd_product_type'] != SellerProduct::PRODUCT_TYPE_ADDON && !$deliveredMarkedByBuyer) {
