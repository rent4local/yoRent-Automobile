...
<div class="scroll scroll-x js-scrollable table-wrap">
    <?php $arr_flds = array(
        ...
        'inventory_value'    =>    Labels::getLabel('LBL_Inventory_Value', $siteLangId) . '<br/>(' . Labels::getLabel('LBL_Stock_Available', $siteLangId) . ' * ' . Labels::getLabel('LBL_Cost_Price', $siteLangId) . ')',
        ...
        foreach ($arr_flds as $key => $val) {
            ...
            switch ($key) {
                ...
                case 'name':
                    ...
                    if ($listing['selprod_sku'] != '') {
                        $name .= '<div class="item__brand">' . Labels::getLabel('LBL_Sku', $siteLangId) . ": </strong>" . $listing['selprod_sku'] . '</div>';
                    }
                    $variantStr = '';
...
$pagingArr = array('pageCount' => $pageCount, 'page' => $page, 'recordCount' => $recordCount, 'callBackJsFunc' => 'goToProductsInventoryStockStatusPage', 'siteLangId' => $siteLangId, 'pageSize' => $pageSize, 'removePageCentClass' => true);
