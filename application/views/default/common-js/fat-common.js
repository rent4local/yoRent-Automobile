siteConstants.userWebRoot = siteConstants.rewritingEnabled
  ? siteConstants.webroot
  : siteConstants.webroot_traditional;
...
var fcom = {
  ...
  logAjaxRequest: function (url, data, res, ajaxLoopHandler) {
    ...
    if (ajaxLoopHandler && repeatCount >= 2) {
      console.log(
        "Executing ajaxLoopHandler for url: " +
          url +
          ", data: " +
          obj.data +
          ", res: " +
  ...
  ajax: function (url, data, fn, options) {
    ...
    var dbmsg =
      o.dbmsg ||
      '<img src="' + fcom.makeUrl() + 'images/loading.gif" alt="Processing..">';
    ...
    $.ajax({
      ...
      error: function (jqXHR, textStatus, error) {
        ...
        console.log(
          "Ajax Request " + url + " error: " + textStatus + " -- " + error
        );
        if (options.errorFn) {
          options.errorFn();
        }
