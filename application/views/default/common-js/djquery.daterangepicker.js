    ...
    $.fn.dateRangePicker = function(opt) {
        ...
        opt = $.extend(true, {
            ...
            hoveringTooltip: function(days, startTime, hoveringTime) {
                return days > 0 ? days + ' ' + translate('days') : '';
            },
            showTopbar: true,
            swapTime: false,
        ...
        function handleEnd(time, startTime) {
            var r = time;
            if (opt.batchMode === 'week-range') {
                var weekDiff = calculateNumberOfWeeks(startTime, parseInt(time));
                r = moment(parseInt(opt.start)).add(((weekDiff * 7) - 1), 'days').valueOf();
                /* if (opt.startOfWeek === 'monday') {
                ...
                var monthsDiff = calculateNumberOfMonths(startTime, parseInt(time));
                r = moment(parseInt(opt.start)).add(((monthsDiff * 30) - 1), 'days').valueOf();
        ...
        function dayClicked(day) {
            ...
            if (opt.singleDate) {
                ...
                if (opt.start > time) {
                    ...
                    opt.end = handleEnd(time, opt.start);
                }
        ...
        function isValidTime(time) {
            ...
            if (opt.start && !opt.end && !opt.singleDate) {
                ...
                if (opt.maxDays > 0 && time > opt.start && countDays(time, opt.start) > opt.maxDays) return false;
                if (opt.minDays > 0 && time > opt.start && countDays(time, opt.start) < opt.minDays) return false;
            ...
            if (opt.start && !opt.end) {
                box.find('.day.toMonth.valid').each(function() {
                    ...
                    if (!isValidTime(time)) {
                        ...
                        if (opt.minDays > 0 && time > opt.start && countDays(time, opt.start) < opt.minDays) {
                            $(this).addClass('invalid tmp isMinDayCheck').removeClass('valid');
                        }
                    } else {
                        $(this).addClass('valid tmp').removeClass('invalid isMinDayCheck');
                        if (opt.minDays > 0 && time > opt.start && countDays(time, opt.start) == opt.minDays) {
                            $(this).addClass('valid tmp isMinDayCheck');
                        }
                    }
                });
            }
        ...
        function dayHovering(day) {
            ...
            var tempEnd = false;
            if (opt.start != false && hoverTime > opt.start && opt.batchMode) {
                if (opt.batchMode === 'week-range') {
                    var weekDiff = calculateNumberOfWeeks(opt.start, hoverTime);
                    var tempEnd  = moment(opt.start).add((weekDiff * 7 - 1), 'days').valueOf();
                } else if(opt.batchMode === 'month-range') {
                    var monthDiff = calculateNumberOfMonths(opt.start, hoverTime);
                    var tempEnd  = moment(opt.start).add(((monthDiff * 30) - 1), 'days').valueOf();
                }
            }
            var tooltip = '';
            if (day.hasClass('has-tooltip') && day.attr('data-tooltip')) {
                ...
                if (opt.singleDate) {
                    ...
                    box.find('.day').each(function() {
                        ...
                        if (opt.batchMode) {
                            if (tempEnd != false && time > opt.start && time <= tempEnd && opt.start != false && !opt.end) {
                            $(this).addClass('hovering'); 
                            } else {
                                $(this).removeClass('hovering');
                            }
                    ...
                    if (opt.start && !opt.end) {
                        ...
                        if (opt.hoveringTooltip) {
                            ...
                            } else if (opt.hoveringTooltip === true && days > 0) {
        ...
        function createDom() {
            ...
            if (opt.showTopbar) {
                ...
                html += '<input type="button" name="closebtn" class="apply-btn disabled' + getApplyBtnClass() + '" value="' + translate('apply') + '" />';
                html += '</div>';
            }
        ...
        function createMonthHTML(d) {
            ...
            for (var week = 0; week < 6; week++) {
                ...
                for (var day = 0; day < 7; day++) {
                    ...
                    var myCustomClass = '';
                    if (opt.start && opt.minDays > 0 && today.time > opt.start && countDays(today.time, opt.start) < opt.minDays) {
                        myCustomClass = ' isMinDayCheck ';
                    }

                    var todayDivAttr = {
                        time: today.time,
                        'data-tooltip': today.tooltip,
                        'class': 'day ' +  myCustomClass + '  ' + today.type + ' ' + today.extraClass + ' ' + (today.valid ? 'valid' : 'invalid') + ' ' + (highlightToday ? 'real-today' : '')
                    };
        ...
        function calculateNumberOfWeeks(date1, date2) {
            return Math.abs(moment(parseInt(date1)).diff(moment(parseInt(date2)), 'weeks')) + 1;
        }
        
        function calculateNumberOfMonths(date1, date2) {
            return Math.abs(moment(parseInt(date1)).diff(moment(parseInt(date2)), 'months')) + 1;
        }
    };
}));
