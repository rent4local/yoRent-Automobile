...
if (!empty($attachments)) {
    ...
    foreach ($attachments as $sn => $row) {
        ...
        //$icon = '<a target="_blank" href="' . UrlHelper::generateUrl('CounterOffer', 'downloadDigitalFile', array($rfqId, $attachmentId, AttachedFile::FILETYPE_QUOTED_DOCUMENT)) . '"><span id="document-js-' . $attachmentId . '">' . $row["afile_name"] . '<i class="fa fa-download"></i>' . '</span></a>';
        $documentUrl = UrlHelper::generateUrl('CounterOffer', 'downloadDigitalFile', [$rfqId, $attachmentId, AttachedFile::FILETYPE_QUOTED_DOCUMENT, true, 70, 70]);
        echo "<div class='uploaded--documents-item' id='document-js-". $attachmentId ."'>";
        if (in_array($ext, $imageType)) {
            ?>
            <a href="<?php echo UrlHelper::generateUrl('CounterOffer', 'downloadDigitalFile', [$rfqId, $attachmentId, AttachedFile::FILETYPE_QUOTED_DOCUMENT]); ?>" title="<?php echo Labels::getLabel('LBL_Download_file', $siteLangId); ?>">
        ...
        if ($isSeller && ((!isset($rfqDetail['rfq_status'])) || isset($rfqDetail['rfq_status']) && $rfqDetail['rfq_status'] == RequestForQuote::REQUEST_INPROGRESS)) {
            ...
            echo '<p class="doc-title"><a href="' . UrlHelper::generateUrl('CounterOffer', 'downloadDigitalFile', [$rfqId, $attachmentId, AttachedFile::FILETYPE_QUOTED_DOCUMENT]) . '"><span>' . $icon = $row["afile_name"] . '</span></a></p>';
        }
        echo "</div>";
    }
} else {
