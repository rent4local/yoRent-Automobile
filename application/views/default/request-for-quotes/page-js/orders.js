...
(function() {
	searchOrders = function(frm){
		...
		fcom.ajax(fcom.makeUrl('RequestForQuotes','orderSearchListing'), data, function(res){
			$("#ordersListing").html(res);
		});
	};
	
	addItemsToCart = function(orderId){
		fcom.ajax(fcom.makeUrl('RequestForQuotes','addItemsToCart',[orderId]), '', function(ans){
