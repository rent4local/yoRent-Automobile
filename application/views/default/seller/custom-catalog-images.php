...
$(function() {
    $("#sortable").sortable({
        stop: function() {
            ...
            fcom.updateWithAjax(fcom.makeUrl('Seller', 'setCustomCatalogProductImagesOrder'), data,
                function(t) {
...
<ul id="sortable" class="inline-images"> <?php
        ...
        <img src="<?php echo UrlHelper::getCachedUrl(UrlHelper::generateFileUrl('image', 'customProduct', array($row['afile_record_id'], "THUMB", $row['afile_id']), CONF_WEBROOT_URL), CONF_IMG_CACHE_TIME, '.jpg'); ?>"
            title="<?php echo $row['afile_name']; ?>" alt="<?php echo $row['afile_name']; ?>"> <a class="close-layer"
            ...
            class="delete"> </a>
        <div class="detail">
            <?php echo ($count == 1) ? '<p><strong>' . Labels::getLabel('LBL_Main_Photo', $siteLangId) . '</strong></p>' : '&nbsp;'; ?></i></a> <?php
...
<ul class="inline-images">
    ...
    <li id="<?php echo $row['afile_id']; ?>">
        <img src="<?php echo FatCache::getCachedUrl(CommonHelper::generateUrl('image', 'customProductSizeChart', array($row['afile_record_id'], "THUMB", $row['afile_id']), CONF_WEBROOT_URL), CONF_IMG_CACHE_TIME, '.jpg'); ?>"
        ...
        <div class="detail">
            <?php
                    ...
                    if ($row['afile_lang_id'] > 0) {
                        ...
                        ?> <?php }
                    ?>
            <p><strong><?php echo Labels::getLabel('LBL_Type', $siteLangId); ?>:</strong>
                <?php echo Labels::getLabel('LBL_Size_Chart', $siteLangId); ?></p>
            <p><strong><?php echo Labels::getLabel('LBL_Language', $siteLangId); ?>:</strong> <?php echo $lang_name; ?>
            </p>
        </div>
    </li>
    <?php } ?>
