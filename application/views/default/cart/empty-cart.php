...
<div class="block--empty m-auto text-center">
    <div class="empty-cart"> <svg viewBox="656 573 264 182" version="1.1" xmlns="http://www.w3.org/2000/svg"
            ...
            <polygon id="diamond" stroke="#C46F82" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                fill="none" points="886 705 889 708 886 711 883 708"></polygon>
            <path
    ...
    <a href="<?php echo UrlHelper::generateUrl('Home'); ?>"
        class="btn btn-brand"><?php echo Labels::getLabel('LBL_Go_To_Homepage',$siteLangId); ?></a>

    <?php if( $EmptyCartItems ) { ?>
    <ul class="links--inline mt-5">
        <?php
		$counter = 1;
		...
		?>
        <li><a target="<?php echo $item['emptycartitem_url_is_newtab']?"_blank":"_self"?>"
                href="<?php echo $itemUrl; ?>"><?php echo $item['emptycartitem_title']; ?></a>
            <?php echo ( $counter < count($EmptyCartItems) ) ? '|' : ''; ?> </li>
        <?php
			$counter ++;
		} ?>
    </ul>
