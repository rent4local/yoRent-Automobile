...
class UserFavoriteProductSearch extends SearchBase
{
    ...
    public function joinSellerproducts($langId = 0, $isActive = true, $isDeleted = true)
    {
        ...
        $this->joinTable(SellerProduct::DB_TBL_SELLER_PROD_DATA, 'INNER JOIN', 'spd.sprodata_selprod_id = sp.selprod_id', 'spd');
        ...
        if ($isActive) {
            $cnd = $this->addCondition('sprodata_rental_active', '=', applicationConstants::ACTIVE);
            if (ALLOW_SALE) {
                $cnd->attachCondition(SellerProduct::DB_TBL_PREFIX . 'active', '=', applicationConstants::ACTIVE, 'OR');
            }
        }
        if ($isDeleted) {
