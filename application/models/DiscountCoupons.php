...
class DiscountCoupons extends MyAppModel
{
    ...
    public static function getSearchObject($langId = 0, $active = true, $isDeleted = true)
    {
        ...
        if ($langId > 0) {
            $srch->joinTable(
                    static::DB_TBL_LANG, 'LEFT OUTER JOIN', 'couponlang_coupon_id = dc.coupon_id AND couponlang_lang_id = ' . $langId, 'dc_l'
            );
        }
    ...
    public static function getCouponPlans($coupon_id, $lang_id = 0)
    {
        ...
        $srch->joinTable(
                SellerPackages::DB_TBL, 'LEFT OUTER JOIN', 'sp.spackage_id = spp.spplan_spackage_id ', 'sp'
        );
        $srch->addMultipleFields(array("spplan_id", "spackage_identifier"));
        if ($lang_id) {
            $srch->joinTable(
                    SellerPackages::DB_TBL . '_lang', 'LEFT OUTER JOIN', 'spl.spackagelang_spackage_id = sp.spackage_id AND spl.spackagelang_lang_id = ' . CommonHelper::getlangId(), 'spl'
            );
    ...
    public static function getValidCoupons($userId, $langId, $coupon_code = '', $orderId = '')
    {
        ...
        foreach ($cartProducts as $cartProduct) {
            if (isset($cartProduct['sellerProdType']) && $cartProduct['sellerProdType'] == SellerProduct::PRODUCT_TYPE_ADDON) {
                continue;
            }
        ...
        foreach ($cartProducts as $cartProduct) {
            ...
            if (1 > FatUtility::int($cartProduct['shop_id'])) {
                continue;
            }
        ...
        foreach ($cartProducts as $cartProduct) {
            ...
            if (1 > FatUtility::int($cartProduct['brand_id'])) {
                continue;
            }
