...
class OptionValue extends MyAppModel
{
    ...
    public function getOptionValue($optionId)
    {
        ...
        if (!empty($record)) {
			...
			/* ADD OPTION MISSING LANGUAGE DATA */
			$defaultLangId = FatApp::getConfig('CONF_DEFAULT_SITE_LANG', FatUtility::VAR_INT, 1);
			$langs = Language::getAllNames();
			$optionNames = $lang_record['optionvalue_name'];
			foreach($langs as $langId => $val ) {
				if (!isset($optionNames[$langId])) {
					$newValue = (isset($optionNames[$defaultLangId])) ? $optionNames[$defaultLangId] : '';
					$lang_record['optionvalue_name'][$langId] = $newValue;
					$lang_record['optionvalue_name'.$langId] = $newValue;
				}
			}
			/* ] */
