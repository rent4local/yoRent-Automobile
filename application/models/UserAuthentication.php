...
{
    public const SESSION_ELEMENT_NAME = 'yorentUserSession';
    public const AFFILIATE_SESSION_ELEMENT_NAME = 'yorentAffiliateSession';
    public const YORENTUSER_COOKIE_NAME = '_uyorent';
    public const TEMP_SESSION_ELEMENT_NAME = 'yorentTempUserSession';
    ...
    {
        $cookieName = self::YORENTUSER_COOKIE_NAME;
    ...
    {
        if (!isset($_COOKIE[static::YORENTUSER_COOKIE_NAME])) {
            return false;
        }

        $db = FatApp::getDb();
        if (strlen($_COOKIE[static::YORENTUSER_COOKIE_NAME])) {
            $db->deleteRecords(
                static::DB_TBL_USER_AUTH,
                array(
                    'smt' => static::DB_TBL_UAUTH_PREFIX . 'token = ?',
                    'vals' => array($_COOKIE[static::YORENTUSER_COOKIE_NAME])
                )
            );
        }

        setcookie($_COOKIE[static::YORENTUSER_COOKIE_NAME], '', time() - 3600, CONF_WEBROOT_URL);
        return true;
    }
