        ...
        switch ($type) {
            ...
            case 'rental':
                ...
                foreach ($last12Months as $key => $val) {
                    $srchObj = clone $srch;
                    $srchObj->addDirectCondition("month(`order_date_added` ) = $val[monthCount] and year(`order_date_added` )= $val[year]");
                    $srchObj->addMultipleFields(array('SUM((opd_rental_price * op_qty) + COALESCE(op_other_charges,0)) AS Sales,avg((opd_rental_price * op_qty ) + COALESCE(op_other_charges,0)) AS avg_order,count(op_id) as total_orders'));
                    $rs = $srchObj->getResultSet();
                    //$srchObj->getQuery(); die('====');

                    $row = $this->db->fetch($rs);
                    $rentalData[] = array("duration" => Labels::getLabel('LBL_' . $val['monthShort'], $langId) . "-" . $val['year'], "value" => round($row["Sales"], 2));
                }
                return $rentalData;
                break;
            case 'rentalearnings':
                ...
                foreach ($last12Months as $key => $val) {
                    ...
                    $rs = $srchObj->getResultSet();
                    $row = $this->db->fetch($rs);
                    $rentalEarningData[] = array("duration" => Labels::getLabel('LBL_' . $val['monthShort'], $langId) . "-" . $val['year'], "value" => round($row["Earning"], 2));
                }
                return $rentalEarningData;
                break;
                /* RENTAL UPDATES ] */
        }
    }
