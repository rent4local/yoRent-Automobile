...
class CatalogReportController extends AdminBaseController
{
    ...
    public function search($type = false)
    {
        ...
        $opSaleSrch->addMultipleFields(
                array('SUBSTRING(op_selprod_code, 1, (LOCATE( "_", op_selprod_code ) - 1 )) as sellerSaleProdId', 'COUNT(op_order_id) as totOrders', 'SUM(op_qty - op_refund_qty) as totSoldQty', 'opd_sold_or_rented')
        );
        ...
        if ($productFor == applicationConstants::PRODUCT_FOR_RENT) {
            ...
            $opRentalSrch->addMultipleFields(
                    array('SUBSTRING(op_selprod_code, 1, (LOCATE( "_", op_selprod_code ) - 1 )) as sellerRentProdId', 'COUNT(op_order_id) as totRentalOrders', 'SUM(op_qty) as totRentedQty', 'opd_sold_or_rented')
            );
        ...
        $opSrch->addMultipleFields(
                array(
                    ...
                    'SUM( ((op_unit_price) * op_qty) - (op_unit_price * op_refund_qty) ) as total',
                    '(SUM(opship.opcharge_amount - op_refund_shipping)) as shippingTotal',
                    '(SUM(optax.opcharge_amount - (optax.opcharge_amount/op_qty * op_refund_qty))) as taxTotal',
        ...
        $srch->addMultipleFields(
                array(
                    ...
                    'opq.total', 'opq.shippingTotal', 'opq.taxTotal', 'opq.commission'
                )
        );
        ...
        if ($productFor == applicationConstants::PRODUCT_FOR_SALE) {
            ...
            $srch->addFld(['IFNULL(opSaleQry.totOrders, 0) as totOrders', 'IFNULL(opSaleQry.totSoldQty, 0) as totSoldQty']);
        }
        ...
        if ($productFor == applicationConstants::PRODUCT_FOR_RENT) {
            ...
            $srch->addFld(
                    ['IFNULL(totRentedQty, 0) as totRentedQty', 'IFNULL(rentalSecurity, 0) as rentalSecurity',
                        'IFNULL(totRentalOrders, 0) as totRentalOrders']
            );
        }
        ...
        if ($type == 'export') {
            ...
            while ($row = $db->fetch($rs)) {
                ...
                if ($productFor == applicationConstants::PRODUCT_FOR_RENT) {
                    ...
                    $arr = array(
                        $name, $row['totRentalOrders'], $row['totRentedQty'],
                        $total, $shipping, $tax, $rentalSecurity, $subTotal, $commission
                    );
                } else {
                    $subTotal = $row['total'] + $row['shippingTotal'] + $row['taxTotal'];
                    $subTotal = CommonHelper::displayMoneyFormat($subTotal, true, true);
                    $arr = array(
                        $name, $row['totOrders'], $row['totSoldQty'],
                        $total, $shipping, $tax, $subTotal, $commission
                    );
                }
                array_push($sheetData, $arr);
            }
