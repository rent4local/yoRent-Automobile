...
class OrderReturnRequestsController extends AdminBaseController
{
    ...
    {
        $this->objPrivilege->canViewOrderReturnRequests();
        $frmSearch = $this->getOrderReturnRequestSearchForm($this->adminLangId, applicationConstants::ORDER_TYPE_RENT);
        $data = FatApp::getPostedData();
        if ($data) {
            $data['orrequest_id'] = FatUtility::int($data['id']);
            unset($data['id']);
        ...
        $this->_template->render(true, true, 'order-return-requests/index.php');
    }
    ...
    public function search()
    {
        ...
        $frmSearch = $this->getOrderReturnRequestSearchForm($this->adminLangId, $orderType);
        $data = FatApp::getPostedData();
        ...
        $srch->addMultipleFields(
                array('orrequest_id', 'orrequest_qty', 'orrequest_type', 'orrequest_returnreason_id',
                    'orrequest_date', 'orrequest_status', 'orrequest_reference', 'buyer.user_name as buyer_name', 'buyer.user_id as buyer_user_id', 'buyer_cred.credential_username as buyer_username',
                    'buyer_cred.credential_email as buyer_email', 'buyer.user_dial_code', 'buyer.user_phone as buyer_phone', 'seller.user_name as seller_name', 'seller.user_id as seller_user_id',
                    'seller.user_dial_code', 'seller.user_phone as seller_phone', 'seller_cred.credential_username as seller_username', 'seller_cred.credential_email as seller_email',
                    'IFNULL(op_product_name, op_product_identifier) as op_product_name', 'IFNULL(op_selprod_title, op_product_identifier) as op_selprod_title', 'op_selprod_options', 'op_brand_name', 'op_shop_id', 'op_shop_name', 'op_qty', 'op_unit_price', 'order_tax_charged', 'op_other_charges', 'op_refund_shipping', 'op_invoice_number')
        );
        ...
        if (isset($post['product']) && $post['product'] != '') {
            ...
            $cnd->attachCondition('op_product_identifier', 'LIKE', "%" . $product . "%");
        ...
        if (isset($post['ref_no']) && $post['ref_no'] != '') {
            $ref_no = trim(FatUtility::convertToType($post['ref_no'], FatUtility::VAR_STRING));
    ...
    public function view($orrequest_id)
    {
        ...
        $srch->addMultipleFields(
                array('orrequest_id', 'orrequest_op_id', 'orrequest_qty', 'orrequest_type', 'orrequest_returnreason_id',
                    'orrequest_date', 'orrequest_status', 'orrequest_reference', 'buyer.user_name as buyer_name', 'buyer.user_id as buyer_user_id', 'buyer_cred.credential_username as buyer_username', 'buyer_cred.credential_email as buyer_email', 'buyer.user_dial_code', 'buyer.user_phone as buyer_phone', 'seller.user_name as seller_name', 'seller.user_id as seller_user_id', 'seller.user_dial_code', 'seller.user_phone as seller_phone', 'seller_cred.credential_username as seller_username', 'seller_cred.credential_email as seller_email', 'IFNULL(op_product_name, op_product_identifier) as op_product_name', 'IFNULL(op_selprod_title, op_product_identifier) as op_selprod_title', 'op_selprod_options', 'op_brand_name', 'op_shop_id', 'op_shop_name', 'op_qty', 'op_unit_price', 'IFNULL(orreason_title, orreason_identifier) as orreason_title', 'order_tax_charged', 'op_other_charges', 'op_refund_shipping', 'op_refund_amount', 'op_commission_percentage', 'op_affiliate_commission_percentage', 'op_commission_include_shipping', 'op_commission_include_tax', 'op_free_ship_upto', 'op_actual_shipping_charges', 'order_pmethod_id', 'op_rounding_off', 'op_invoice_number', 'opd_rental_security', 'opd_sold_or_rented', 'op_commission_charged')
        );
        ...
        $this->set('isRentalOrder', ($requestRow['opd_sold_or_rented'] == applicationConstants::ORDER_TYPE_RENT) ? true : false);
        $this->_template->render();
    }
    ...
    private function getOrderReturnRequestSearchForm(int $langId, int $orderType = applicationConstants::ORDER_TYPE_SALE)
    {
    ...
    private function getUpdateStatusForm($orrequest_id, $langId, $canRefundToCard = false)
    {
        ...
        $frm->addHiddenField('', 'orrequest_refund_in_wallet', PaymentMethods::MOVE_TO_CUSTOMER_WALLET);
