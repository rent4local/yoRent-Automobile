...
class SalesReportController extends AdminBaseController
{
    ...
    public function search()
    {
        ...
        $srch = Report::salesReportObject($this->adminLangId, false, array(), applicationConstants::ORDER_TYPE_SALE);
        if (empty($orderDate)) {
    ...
    public function export()
    {
        ...
        $post = $srchFrm->getFormDataFromArray(FatApp::getPostedData());
        $srch = Report::salesReportObject($this->adminLangId, false, [], applicationConstants::PRODUCT_FOR_SALE);
        if (empty($orderDate)) {
            $date_from = FatApp::getPostedData('date_from', FatUtility::VAR_DATE, '');
            if (!empty($date_from)) {
                $srch->addCondition('o.order_date_added', '>=', $date_from . ' 00:00:00');
