...
class BlogPostsController extends AdminBaseController
{
    ...
    public function imagesForm($post_id)
    {
        ...
        $featuredImageFrm = $this->getFeaturedImagesFrm($post_id);
        $this->set('languages', Language::getAllNames());
        $this->set('post_id', $post_id);
        $this->set('imagesFrm', $imagesFrm);
        $this->set('featuredImageFrm', $featuredImageFrm);
        $this->_template->render(false, false);
    }
    ...
    public function featuredImage(int $postId, int $langId = 0)
    {
        $this->objPrivilege->canViewBlogPosts();
        ...
        if (!$row = BlogPost::getAttributesById($postId)) {
            FatUtility::dieWithError($this->str_no_record);
        }
        $featuredImage = AttachedFile::getAttachment(AttachedFile::FILETYPE_BLOG_POST_FEATURED_IMAGE, $postId, 0, $langId, false);
        $this->set('languages', Language::getAllNames());
        $this->set('images', $featuredImage);
        $this->set('post_id', $postId);
        $this->_template->render(false, false);
    }
    ...
    public function uploadBlogPostImages($post_id, $lang_id = 0)
    {
        ...
        $uniqueRecord = false;
        if ($file_type == AttachedFile::FILETYPE_BLOG_POST_FEATURED_IMAGE) {
            $uniqueRecord = true;
        }

        $allowedFileTypeArr = array(AttachedFile::FILETYPE_BLOG_POST_IMAGE, AttachedFile::FILETYPE_BLOG_POST_FEATURED_IMAGE);
        ...
        if (!$res = $fileHandlerObj->saveAttachment($_FILES['cropped_image']['tmp_name'], $file_type, $post_id, 0, $_FILES['cropped_image']['name'], -1, $uniqueRecord, $lang_id)) {
    ...
    private function getFeaturedImagesFrm($post_id = 0)
    {
        $sizeArr = imagesSizes::blogImageSizeArr()[applicationConstants::getActiveTheme()];
        $this->objPrivilege->canViewBlogPosts();
        $bannerTypeArr = applicationConstants::bannerTypeArr();

        $frm = new Form('frmBlogPostFeaturedImage', array('id' => 'featuredImageFrm'));
        $frm->addHiddenField('', 'post_id', $post_id);
        $frm->addSelectBox(Labels::getLabel('LBL_Language', $this->adminLangId), 'lang_id', $bannerTypeArr, '', array(), '');
        $frm->addHiddenField('', 'file_type', AttachedFile::FILETYPE_BLOG_POST_FEATURED_IMAGE);
