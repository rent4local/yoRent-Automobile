...
class ShopsController extends AdminBaseController
{
    ...
    public function shopAgreement(int $shopId = 0) 
    {
        if ($shopId < 1) {
        ...
        $this->set("attachment", AttachedFile::getAttachment(AttachedFile::FILETYPE_SHOP_AGREEMENT, $shopId, 0, -1));
        $this->set('frmShopAgreement', $this->getShopAgreement($shopId));
    ...
    {
        $aFileId = FatApp::getPostedData('id', FatUtility::VAR_INT, 0);
        ...
        if ($this->checkAgreementAttWithOrder($aFileId)) {
            if (!AttachedFile::updateFileStatus(AttachedFile::FILETYPE_SHOP_AGREEMENT, $shopId, 0, $aFileId)) {
                Message::addErrorMessage(Labels::getLabel('MSG_Invalid_Request', $this->adminLangId));
                FatUtility::dieJsonError(Message::getHtml());
            }
        } else {
            $attchObj = new AttachedFile();
            if (!$attchObj->deleteFile(AttachedFile::FILETYPE_SHOP_AGREEMENT, $shopId, $aFileId)) {
                Message::addErrorMessage(Labels::getLabel('MSG_Error_in_deleting_file', $this->adminLangId));
                FatUtility::dieJsonError(Message::getHtml());
            }
        }
    ...
    public function checkAgreementAttWithOrder(int $aFileId) : bool
    {
        if (1 > $aFileId) {
            return false;
        }
        $srch = Orders::getOrderProductSearchObject();
        $srch->addCondition('opd_rental_agreement_afile_id', '=', $aFileId);
    ...
    public function setupShopAgreement()
    {
        $shopId = FatApp::getPostedData('shop_id', FatUtility::VAR_INT, 0);
        if ($shopId < 1) {
            Message::addErrorMessage(Labels::getLabel('MSG_invalid_shop_id', $this->adminLangId));
            FatUtility::dieWithError(Message::getHtml());
        }
        ...
        if (!$res = $fileHandlerObj->saveAttachment($_FILES['shop_agreemnet']['tmp_name'], AttachedFile::FILETYPE_SHOP_AGREEMENT, $shopId, 0, $_FILES['shop_agreemnet']['name'], -1, false, $this->adminLangId)) {
    ...
    private function getShopAgreement(int $shopId = 0)
    {
        $frm = new Form('frmShopAgreement');
        $frm->addHiddenField('', 'shop_id', $shopId);
        $frm->addFileUpload(Labels::getLabel('LBL_Upload', $this->adminLangId), 'shop_agreemnet', array('accept' => 'application/pdf', 'data-frm' => 'frmShopBanner'));
