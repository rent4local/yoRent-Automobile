...
class BrandsController extends AdminBaseController
{
    ...
    public function changeRequestStatus()
    {
        ...
        $data = Brand::getAttributesById($brandId, array('brand_id', 'brand_identifier', 'brand_seller_id'));
        ...
        $urlSrch->addCondition('urlrewrite_original', '=', $this->rewriteUrl . $brandId);
    ...
    public function setupRequestData($post)
    {
        $brand_id = $post['brand_id'];
        ...
        if (empty($brandLangData['brand_name'])) {
            $brandData['brand_name'] = $brandData['brand_identifier'];
        } else {
        ...
        Product::updateMinPrices(0, 0, $brandId);
        if($brandStatus == ProductCategory::REQUEST_CANCELLED) {
            $msg = Labels::getLabel('MSG_Brand_Request_Cancelled', $this->adminLangId);
        }else{
           $msg = Labels::getLabel('MSG_Brand_Setup_Successful', $this->adminLangId);
        }
        $this->set('msg', $msg);
        $this->set('brandId', $brandId);
        $this->set('langId', $newTabLangId);
    ...
    public function media($brand_id = 0, $langId = 0, $slide_screen = 0)
    {
        ...
        $brandFeaturedImgFrm = $this->getBrandFeaturedImageForm($brand_id);
        $data['lang_id'] = $langId;
        $data['ratio_type'] = AttachedFile::RATIO_TYPE_SQUARE;
        if (0 < $brand_id) {
            $brandFeaturedImg = current(AttachedFile::getMultipleAttachments(AttachedFile::FILETYPE_BRAND_FEATURED_IMAGE, $brand_id, 0, $langId, false));
            if (is_array($brandFeaturedImg) && count($brandFeaturedImg)) {
                $data['ratio_type'] = (isset($brandLogo['afile_aspect_ratio'])) ? $brandLogo['afile_aspect_ratio'] : AttachedFile::RATIO_TYPE_SQUARE;
            }
        }
        $brandFeaturedImgFrm->fill($data);

        $this->set('languages', Language::getAllNames());
        $this->set('brand_id', $brand_id);
        $this->set('brandLogoFrm', $brandLogoFrm);
        $this->set('brandImageFrm', $brandImageFrm);
        $this->set('brandfFeaturedImgFrm', $brandFeaturedImgFrm);
        $this->_template->render(false, false);
    }
    ...
    public function images($brand_id, $file_type, $lang_id = 0, $slide_screen = 0)
    {
        ...
        } elseif ($file_type == 'featuredImage') {
            $brandLogos = AttachedFile::getMultipleAttachments(AttachedFile::FILETYPE_BRAND_FEATURED_IMAGE, $brand_id, 0, $lang_id, false);
            $this->set('images', $brandLogos);
            $this->set('imageFunction', 'brandFeaturedImage');
        } else {
    ...
    public function uploadMedia()
    {
        ...
        if (!$res = $fileHandlerObj->saveAttachment($_FILES['cropped_image']['tmp_name'], $file_type, $brand_id, 0, $_FILES['cropped_image']['name'], -1, $unique_record = false, $lang_id, $slide_screen, $aspectRatio)) {
    ...
    {
        if ($attachment = AttachedFile::getAttachment(AttachedFile::FILETYPE_BRAND_LOGO, $brandId, 0, 0, true, 0, false)) {
            return true;
        }
        return false;
    }
    ...
    public function getBrandImageForm($brand_id)
    {
        ...
        $frm->addHTML('', Labels::getLabel('LBL_Banner_Image', $this->adminLangId), '<h3>' . Labels::getLabel('LBL_Banner_Image', $this->adminLangId) . '</h3>');
        $languagesAssocArr = Language::getAllNames();
        $frm->addHTML('', 'brand_logo_heading', '');
        $frm->addHiddenField('', 'brand_id', $brand_id);
        $frm->addSelectBox(Labels::getLabel('LBL_Language', $this->adminLangId), 'lang_id', array(0 => Labels::getLabel('LBL_Universal', $this->adminLangId)) + $languagesAssocArr, '', array(), '');
    ...
    public function getBrandFeaturedImageForm($brand_id)
    {
        $sizeArr = imagesSizes::brandFeaturedImageSizeArr()[applicationConstants::getActiveTheme()];
        $frm = new Form('frmBrandFeaturedImage');
        $frm->addHTML('', Labels::getLabel('LBL_Featured_Image', $this->adminLangId), '<h3>' . Labels::getLabel('LBL_Featured_Image', $this->adminLangId) . '</h3>');
        $languagesAssocArr = Language::getAllNames();
        $frm->addHTML('', 'brand_Featured_Image_heading', '');
        $frm->addHiddenField('', 'brand_id', $brand_id);
        $frm->addSelectBox(Labels::getLabel('LBL_Language', $this->adminLangId), 'lang_id', array(0 => Labels::getLabel('LBL_Universal', $this->adminLangId)) + $languagesAssocArr, '', array(), '');
        $screenArr = applicationConstants::getDisplaysArr($this->adminLangId);
        $frm->addSelectBox(Labels::getLabel("LBL_Display_For", $this->adminLangId), 'slide_screen', $screenArr, '', array(), '');
        $frm->addHiddenField('', 'file_type', AttachedFile::FILETYPE_BRAND_FEATURED_IMAGE);
        $frm->addHiddenField('', 'image_min_width', $sizeArr['width']);
        $frm->addHiddenField('', 'image_min_height', $sizeArr['height']);
        $frm->addFileUpload(Labels::getLabel('LBL_Upload', $this->adminLangId), 'featured_image', array('accept' => 'image/*', 'data-frm' => 'frmBrandFeaturedImage'));
        $frm->addHtml('', 'brand_featured_image_display_div', '');
        return $frm;
    }
    ...
    public function removeBrandMedia($brand_id, $imageType = '', $afileId = 0)
    {
        ...
        } elseif ($imageType == 'featuredImage') {
            $fileType = AttachedFile::FILETYPE_BRAND_FEATURED_IMAGE;
        }
        $fileHandlerObj = new AttachedFile();
    ...
    public function searchBrandRequests()
    {
        ...
        $srch->addMultipleFields(array('b.*', 'u.user_name', 'ifnull(shop_name, shop_identifier) as shop_name, shop_id, shop_user_id'));
        ...
        if (!empty($post['keyword'])) {
            $keyword = trim($post['keyword']);
            $condition = $srch->addCondition('b.brand_identifier', 'like', '%' . $keyword . '%');
            $condition->attachCondition('bl.brand_name', 'like', '%' . $keyword . '%', 'OR');
        }
        if (!empty($post['brand_id'])) {
            $srch->addCondition('b.brand_id', '=', $post['brand_id']);
        }
    ...
    public function updateStatusForm($brandId = 0)
    {
        ...
        if (0 < $brandId) {
            $data = Brand::getAttributesById($brandId, array('brand_id', 'brand_identifier', 'brand_active', 'brand_featured', 'brand_status', 'brand_seller_id'));
            if ($data === false) {
                FatUtility::dieWithError($this->str_invalid_request);
            }
            /* url data[ */
    ...
    private function getStatusForm()
    {
        ...
        $fld = $frm->addSelectBox(Labels::getLabel('LBL_brand_status', $this->adminLangId), 'brand_status', $reqStatusArr);
        $fld->requirements()->setRequired();
        $frm->addHiddenField('', 'brand_comments', '', array('placeholder' =>  Labels::getLabel('LBL_Add_Comment', $this->adminLangId)));

        $frm->addHiddenField('', 'brand_id');
        $frm->addHiddenField('', 'brand_identifier');
        $frm->addHiddenField('', 'urlrewrite_custom');
