...
(function() {
	...
	subPages = function (nlink_id, navId){
		fcom.ajax(fcom.makeUrl('Navigations', 'subPages', [nlink_id, navId]), '', function(t) {
			$(dv).html(t);
		});
	};
	...
	addChildNavigationLinkForm = function( nav_id, nlink_id){
		$.facebox(function() {
					childNavigationLinkForm( nav_id, nlink_id);
		});
	}


	childNavigationLinkForm = function( nav_id, nlink_id){
		fcom.displayProcessing();
		var data = 'nav_id=' + nav_id + '&nlink_parent_id=' + nlink_id+ '&nlink_id=0';
			fcom.ajax(fcom.makeUrl('Navigations', 'navigationLinkForm'), data, function(t) {
				fcom.updateFaceboxContent(t)
			});
	}
	...
	callPageTypePopulate = function(el){
		...
		console.log(NAVLINK_TYPE_CATEGORY_PAGE);
		console.log(nlink_type);
		if( nlink_type == 0 ){
			/* if cms Page */
			$("#nlink_url_div").hide();
		...
		if(nlink_type == NAVLINK_TYPE_CATEGORY_PAGE){
			$('#nlink_parent').prop('disabled', true);
		}else{
			$('#nlink_parent').prop('disabled', false);
		}
	};
