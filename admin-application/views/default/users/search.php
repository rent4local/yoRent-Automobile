    ...
    foreach ($arr_flds as $key => $val) {
        ...
        switch ($key) {
            ...
            case 'user':
                ...
                $userDetail .= '<strong>' . Labels::getLabel('LBL_Phone:', $adminLangId) . ' </strong>' . $row['user_dial_code'] . ' ' . $row['user_phone'] . '<br/>';
            ...
            case 'type':
                ...
                if ($str == '' && $row['user_registered_initially_for'] != 0 && isset($arr[$row['user_registered_initially_for']])) {
                    $str = '<span class="label label-danger">Signing Up For: ' . $arr[$row['user_registered_initially_for']] . '</span>';
                }
            ...
            case 'action':
                ...
                if ($canEdit) {
                    ...
                    if (0 == $row['user_parent']) {
                        ...
                        $innerLi = $innerUl->appendElement("li");
                        $innerLi->appendElement('a', array(
                            'href' => 'javascript:void(0)', 'class' => 'button small green',
                            'title' => Labels::getLabel('LBL_Transactions', $adminLangId), "onclick" => "transactions(" . $row['user_id'] . ")"
                        ), Labels::getLabel('LBL_Transactions', $adminLangId), true);
                    }
                    $innerLi = $innerUl->appendElement('li');
