...
<?php if (!empty($images)) { ?>
    ...
    <?php
        ...
        <li id="<?php echo $row['afile_id']; ?>">
          <div class="logoWrap">
            <div class="logothumb"> <img src="<?php echo UrlHelper::generateUrl('image', $imageFunction, array($row['afile_record_id'], $row['afile_lang_id'], "THUMB", $row['afile_id']), CONF_WEBROOT_FRONT_URL); ?>" title="<?php echo $row['afile_name'];?>" alt="<?php echo $row['afile_name'];?>"> <?php if ($canEdit) { ?> <a class="deleteLink white" href="javascript:void(0);" title="Delete <?php echo $row['afile_name'];?>" onclick="deleteImage(<?php echo $row['afile_id']; ?>, <?php echo $row['afile_record_id']; ?>, '<?php echo $imageType; ?>', <?php echo $row['afile_lang_id']; ?>,<?php echo $row['afile_screen'];?>);" class="delete"><i class="ion-close-round"></i></a>
              <?php } ?>
            </div>
