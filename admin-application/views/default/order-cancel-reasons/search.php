...
foreach ($arr_listing as $sn => $row) {
    ...
    foreach ($arr_flds as $key => $val) {
        ...
        switch ($key) {
            case 'select_all':
                $td->appendElement('plaintext', array(), '<label class="checkbox"><input class="selectItem--js" type="checkbox" name="ocreason_ids[]" value=' . $row['ocreason_id'] . '></label>', true);
                break;
            case 'listserial':
...
$frm->setFormTagAttribute('action', UrlHelper::generateUrl('OrderCancelReasons', 'deleteSelected'));
