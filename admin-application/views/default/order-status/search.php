    ...
    foreach ($arr_flds as $key => $val) {
        ...
        switch ($key) {
            case 'dragdrop':
                ...
                break;
            case 'select_all':
                $td->appendElement('plaintext', array(), '<label class="checkbox"><input class="selectItem--js" type="checkbox" name="orderstatus_ids[]" value=' . $row['orderstatus_id'] . '></label>', true);
                break;
            case 'listserial':
            ...
            case 'action':
                if ($canEdit) {
                    $td->appendElement('a', array('href' => 'javascript:void(0)', 'class' => 'btn btn-clean btn-sm btn-icon', 'title' => Labels::getLabel('LBL_Edit', $adminLangId), "onclick" => "editOrderStatusForm(" . $row['orderstatus_id'] . ")"), "<i class='far fa-edit icon'></i>", true);
                }
                break;
            default:
...
$frm->setFormTagAttribute('action', UrlHelper::generateUrl('OrderStatus', 'toggleBulkStatuses'));
